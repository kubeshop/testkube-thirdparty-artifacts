...
mongodb_conf_get() {
    ...
    if [[ -r "$MONGODB_CONF_FILE" ]]; then
        local -r res="$(yq ".${key}" "$MONGODB_CONF_FILE")"
        if [[ ! "$res" = "null" ]]; then
            echo "$res"
        fi
    fi
...
mongodb_configure_primary() {
    ...
    wait-for-it -t 360 -h localhost -p "$MONGODB_PORT_NUMBER"
...
mongodb_wait_for_node() {
    ...
    if ! retry_while "wait-for-it -t 10 -h $host -p $port" "$MONGODB_INIT_RETRY_ATTEMPTS" "$MONGODB_INIT_RETRY_DELAY"; then
